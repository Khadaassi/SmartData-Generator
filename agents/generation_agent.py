...
from validation.schemas import IssueLevel, ValidationReport
...
def _validate_data(state: GenerationState) -> GenerationState:
    request = state["request"]
    run_id = state["run_id"]

    logger.info("[%s] validation des données entity=%s items=%d", run_id, request.entity.name, len(state["items"]))
    report = validate_batch(request.entity, state["items"], request.rules)
    state["validation_report"] = report

    for issue in report.issues:
        prefix = f"[objet {issue.item_index}] " if issue.item_index is not None else ""
        state["errors"].append(
            GenerationError(
                code=issue.code,
                message=f"{prefix}{issue.message}",
                stage="validation",
                blocking=issue.level == IssueLevel.ERROR,
            )
        )

    state["items"] = report.valid_data
    state["status"] = "FAILED" if report.status == "FAILED" else "SUCCESS"

    if report.status == "FAILED":
        logger.error("[%s] validation échouée : aucun objet valide sur %d", run_id, report.total_items)
    elif report.rejected_items:
        logger.warning("[%s] validation : %d/%d objets rejetés", run_id, report.rejected_items, report.total_items)
    else:
        logger.info("[%s] validation réussie : %d objets valides", run_id, report.valid_items)

    return state


def _route_on_status(state: GenerationState) -> Literal["continue", "handle_error"]:
    return "handle_error" if state["status"] == "FAILED" else "continue"
...
def get_generation_graph():
    ...
    graph.add_conditional_edges(
        "generate_data",
        _route_on_status,
        {"continue": "validate_data", "handle_error": "handle_error"},
    )
    graph.add_conditional_edges(
        "validate_data",
        _route_on_status,
